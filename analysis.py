...
def get_portfolio_value(prices, allocs, start_val=1):
    ...
    #Get allocations and start_val
    df = df * allocs * start_val
    
    #Get daily position value
...
def get_portfolio_stats(port_val, daily_rf=0, samples_per_year=252):
    ...
    #Calculate avg_daily_return as mean of dailyreturns
    avg_daily_ret = daily_return.mean()

    std_daily_ret= daily_return.std()

    sharpe_ratio= samples_per_year**(0.5) * daily_return.mean()/daily_return.std();
...
def plot_normalized_data(df, title="Normalized prices", xlabel="Date", ylabel="Normalized price"):
    ...
    if os.path.isdir("output/"):
        filename = "output/comparison.png"
...
def test_run():
    ...
    #symbol allocation for graph
    #Symbols: ['GOOG', 'AAPL', 'GLD', 'XOM'], Allocations: [0.2, 0.2, 0.4, 0.2]
    symbol_allocations = OrderedDict([('GOOG', 0.2), ('AAPL', 0.2), ('GLD', 0.4), ('XOM', 0.2)])
    
    # symbols and corresponding allocations
    

    # symbols and corresponding allocations
    #symbol_allocations = OrderedDict([('GOOG', 0.2), ('AAPL', 0.3), ('GLD', 0.4), ('XOM', 0.1)])
